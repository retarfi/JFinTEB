...
from jfinteb.embedders import TextEmbedder
from jfinteb.evaluators import EmbeddingEvaluator
from jfinteb.utils.score_recorder import JsonScoreRecorder
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        enable_path=True,
        default=str(Path(__file__).parent / "configs" / "jfinteb.jsonnet"),
    )
    ...
    if args.save_dir:
        ...
        parser.save(
            args,
            Path(args.save_dir) / "jfinteb_config.yaml",
            format="yaml",
            overwrite=True,
            multifile=False,
            skip_check=True,
